...
class IntegrationsApi(Protocol):
    ...
    async def list_integrations(
        self, db: DbDep, workspace_id: WorkspaceIdDep, _auth: TpsAuthDep
    ) -> list[dict]:
        """GET /integrations — list all active integrations for a workspace."""
        ...

    async def install_app(
        self, app_name: str, db: DbDep, workspace_id: WorkspaceIdDep, _auth: TpsAuthDep, body: dict | None = None
    ) -> dict:
        """POST /integrations/{app_name}/install — start OAuth flow."""
        ...

    async def exchange_oauth_code(
        self, app_name: str, body: dict, db: DbDep, workspace_id: WorkspaceIdDep, _auth: TpsAuthDep
    ) -> dict:
