...
const STATUS_COLORS: Record<string, string> = {
  ...
  cloning: "bg-[var(--amber)]",
  parsing: "bg-[var(--amber)]",
  generating: "bg-[var(--amber)]",
  packaging: "bg-[var(--amber)]",
  failed: "bg-[var(--rose)]",
  timed_out: "bg-[var(--rose)]",
...
export default function ServicesListPage() {
  ...
  function getRepoShortUrl(url: string | null): string {
    if (!url) return "";
    ...
    <div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-32 bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius)] animate-pulse" />
          ))}
        </div>
      ) : filtered && filtered.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {filtered.map((service) => {
            const isActive = !["failed", "timed_out"].includes(service.status);

            return (
              <Link
                key={service.id}
                href={`/services/${service.id}`}
                className={`bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius)] p-4 flex flex-col gap-2 no-underline hover:border-[rgba(20,184,166,0.2)] hover:-translate-y-0.5 hover:shadow-[0_4px_24px_rgba(0,0,0,0.3)] transition-all duration-200 ${
                  !isActive ? "opacity-60" : ""
                }`}
              >
                {/* Header: name + status */}
                <div className="flex items-center justify-between">
                  ...
                  <span className={`shrink-0 px-2 py-0.5 text-[9px] font-[family-name:var(--font-jetbrains-mono)] font-semibold rounded-full border ${STATUS_BADGE_STYLES[service.status] || STATUS_BADGE_STYLES.pending}`}>
                    {service.status === "parsing" ? "parsing..." : service.status}
                  </span>
                </div>

                {/* Repo URL */}
                {service.repo_url && (
                  <div className="font-[family-name:var(--font-jetbrains-mono)] text-[11px] text-[var(--text-muted)] truncate">
                    {getRepoShortUrl(service.repo_url)}
                  </div>
                )}
                ...
                <div className="flex gap-2 flex-wrap mt-auto">
                  {service.framework && (
                    <span className="px-2 py-0.5 text-[9px] font-[family-name:var(--font-jetbrains-mono)] font-semibold rounded bg-[var(--violet)]/10 border border-[var(--violet)]/20 text-[var(--violet)]">
                      {service.framework}
                    </span>
                  )}
