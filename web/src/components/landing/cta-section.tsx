...
export function CtaSection() {
  ...
  const asciiInView = useInView(asciiRef, { once: false, amount: 0.5 });
  const [showTakeover, setShowTakeover] = useState(false);
  const [asciiRevealed, setAsciiRevealed] = useState(false);
  const hasTriggered = useRef(false);

  // Trigger zoom-to-top after ASCII reveals
  const triggerZoom = useCallback(() => {
    if (hasTriggered.current) return;
    hasTriggered.current = true;

    // Wait for lines to finish revealing
    const totalDelay = ASCII_ART.length * 120 + 800;
    setTimeout(() => {
      setShowTakeover(true);
      // After zoom animation, scroll to top
      setTimeout(() => {
  ...
  // Trigger when ASCII section enters view
  if (asciiInView && !asciiRevealed) {
    ...
    <>
      <section ref={ref} className="min-h-screen py-24 px-6 lg:px-[60px] text-center relative z-[1] flex flex-col items-center justify-center">
        ...
        <div ref={asciiRef} className="text-center mt-8">
          {asciiRevealed && ASCII_ART.map((line, i) => (
            <motion.div
              key={i}
      ...
      {/* Fullscreen zoom takeover */}
      <AnimatePresence>
        {showTakeover && (
          <motion.div
            className="fixed inset-0 z-[9998] bg-[var(--bg)] flex flex-col items-center justify-center pointer-events-none"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.pre
              className="font-[family-name:var(--font-jetbrains-mono)] text-[var(--accent)] whitespace-pre leading-[1.15] text-center"
